        ...
        public ActionResult ShowPurchaseRptInNewWin(string FromDate, string ToDate, int[] LedgerIds, string SelectedReportOption, string ShowType)
        {
            ...
            try
            {
                ...
                if (LedgerIds != null)
                {
                    ...
                    sql = "SELECT dbo.Purchases.PurchaseId, dbo.Purchases.PurchaseDate, dbo.Purchases.PChallanNo, dbo.Purchases.Quantity, dbo.Purchases.SE, dbo.Purchases.Amount, dbo.Purchases.Discount, dbo.Purchases.ShowRoomId, dbo.Purchases.SupplierId, dbo.Suppliers.SupplierName, dbo.Purchases.PurchasedProductId, dbo.PurchasedProducts.PurchasedProductName, dbo.ShowRooms.ShowRoomName, dbo.Purchases.ProcesseLocationId, dbo.Purchases.DeliveryQuantity, dbo.Purchases.ProcessListId FROM dbo.Purchases INNER JOIN dbo.ShowRooms ON dbo.Purchases.ShowRoomId = dbo.ShowRooms.ShowRoomId LEFT OUTER JOIN dbo.Suppliers ON dbo.Purchases.SupplierId = dbo.Suppliers.SupplierId LEFT OUTER JOIN dbo.PurchasedProducts ON dbo.Purchases.PurchasedProductId = dbo.PurchasedProducts.PurchasedProductId WHERE (dbo.Purchases.SupplierId IS NOT NULL) AND(dbo.Purchases.ShowRoomId = @showRoomId) AND(dbo.Purchases.PurchaseDate >= CONVERT(DATETIME, @fromDate, 102)) AND (dbo.Purchases.PurchaseDate <= CONVERT(DATETIME, @toDate, 102)) AND (dbo.Purchases.SupplierId IN (" + inIds + "))";
                }
                else
                {
                    sql = @"SELECT        
                                ...
                                dbo.Purchases.ProcesseLocationId, dbo.Purchases.DeliveryQuantity, dbo.Purchases.ProcessListId
                                FROM            
                                ...
                                WHERE        
                                (dbo.Purchases.SupplierId IS NOT NULL) AND (dbo.Purchases.ShowRoomId = @showRoomId) AND (dbo.Purchases.PurchaseDate >= CONVERT(DATETIME, @fromDate, 102)) AND 
                                (dbo.Purchases.PurchaseDate <= CONVERT(DATETIME, @toDate, 102))";
                }
